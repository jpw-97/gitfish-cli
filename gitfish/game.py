...
async def cast_loop(tick_interval=5):
    """
    Commences idle fishing loop for the specified tick duration.
    """
    ...
    equipped = state.get("equipped_bait", "maggots")

    if inv.get(equipped, 0) <= 0:
        console.print("You have no bait equipped. Equip bait or visit the shop to buy some.")
        return

    inv[equipped] -= 1
    console.print(f"You cast your line out... Currently using {equipped} for bait.")
    ...
    bite_chance = base_bite_chance + (total_mod+(total_mod*BAIT_OPTIONS[equipped]["bite_bonus"]))
    ...
    try:
        while True:
            ...
            if random.random() < bite_chance:
                ...
                fish_difficulty = random.randint(0, 2)
